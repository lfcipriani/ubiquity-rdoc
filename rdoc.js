var TallEye = {
  ...
  preferences: {
    ...
    getRdocs: function() {
      ...
      var temp = {};
      for (var key in tempRdocs) {
        temp[key] = {
          baseUrl:     tempRdocs[key],
          isIndexed:   false,
...
var noun_type_rdoc = {
        ...
        Utils.parseRemoteDocument(
          ...
          function(response) {
            ...
            jQuery("a", response).each(function(i) {
               responseJSON.push({
                 name: this.text,
                 url:  this.attributes[0].nodeValue
               });
            });
  ...
  suggest: function(text, html) {
    ...
    for (var rdoc_name in this._getRdocs()) {
...
TallEye.availableRdocs = function() {
  var result = '<p>Available RDocs:</p><ul>';

  for (var key in noun_type_rdoc._getRdocs()) {
    result += '<li>' + key + '</li>';
  }
...
TallEye.searchOnRdoc = function(searchTerm, rdoc_type, previewBlock) {
  ...
  if (searchTerm.indexOf("#") == -1) {  // when searching for Class_or_method
    var fullIndex = noun_type_rdoc._getRdocs()[rdoc_type].searchIndex['class'].concat(
      noun_type_rdoc._getRdocs()[rdoc_type].searchIndex.method
    );

    for (var term in fullIndex) {
      if (fullIndex[term].name.match(searchTerm, "i")) {
        suggestions.push({
          name: fullIndex[term].name,
          url:  noun_type_rdoc._getRdocs()[rdoc_type].baseUrl + fullIndex[term].url
        });
      }
    }
  }
    ...
    for (var term in methodIndex) {
      ...
      if (indexTerms[2].match(terms[0], "i")) {
        ...
        if (indexTerms[1].match(terms[1], "i")) {
          suggestions.push({
            name: methodIndex[term].name,
            url:  noun_type_rdoc._getRdocs()[rdoc_type].baseUrl + methodIndex[term].url
          });
        }
      }
    }
...
TallEye.createCommand({
  ...
  'arguments': [
    { role: "object",   nountype: noun_arb_text,  label: "class or method" },
...
TallEye.createCommand({
  ...
  'arguments': [
    { role: "object", nountype: noun_type_url, label: "rdoc base URL" },
...
TallEye.createCommand({
  ...
  'arguments': [
    { role: "object", nountype: noun_type_rdoc, label: "rdoc" }
  ],
